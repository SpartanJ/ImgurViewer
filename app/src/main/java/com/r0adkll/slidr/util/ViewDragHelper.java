...
public class ViewDragHelper {
     ...
     */
    public boolean canScrollVertically(View v, boolean checkV, int dx, int dy, int x, int y) {
        if (v instanceof ViewGroup) {
            ...
            for (int i = count - 1; i >= 0; i--) {
                ...
                if (x + scrollX >= child.getLeft() && x + scrollX < child.getRight() &&
					...
					canScrollVertically(child, true, dx, dy, x + scrollX - child.getLeft(),
	...
	public boolean canScrollHorizontally(View v, boolean checkV, int dx, int dy, int x, int y) {
		if (v instanceof ViewGroup) {
			final ViewGroup group = (ViewGroup) v;
			final int scrollX = v.getScrollX();
			final int scrollY = v.getScrollY();
			...
			for (int i = count - 1; i >= 0; i--) {
				...
				if (x + scrollX >= child.getLeft() && x + scrollX < child.getRight() &&
					y + scrollY >= child.getTop() && y + scrollY < child.getBottom() &&
					canScrollHorizontally(child, true, dx, dy, x + scrollX - child.getLeft(),
						y + scrollY - child.getTop())) {
					return true;
				}
			}
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                for (int i = 0; i < pointerCount; i++) {
                    ...
                    final View toCapture = findTopChildUnder((int) x, (int) y);
                    final boolean canScroll = ( ( mTrackingEdges & ( EDGE_LEFT | EDGE_RIGHT ) ) != 0 ) ?
												canScrollHorizontally( toCapture, true, (int)dx, (int)dy, (int)x, (int)y ) :
												canScrollVertically( toCapture, true, (int)dx, (int)dy, (int)x, (int)y );
                    final boolean pastSlop = toCapture != null && checkTouchSlop(toCapture, dx, dy) && !canScroll;
                    if (pastSlop) {
    ...
    private boolean checkTouchSlop(View child, float dx, float dy) {
        ...
        } else if (checkHorizontal && 3 * temp_dy < Math.abs(dx)) {
