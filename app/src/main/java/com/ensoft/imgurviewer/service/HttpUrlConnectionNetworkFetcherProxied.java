...
public class HttpUrlConnectionNetworkFetcherProxied extends BaseNetworkFetcher<HttpUrlConnectionNetworkFetcherProxied.HttpUrlConnectionNetworkFetchState>
{
	...
	public HttpUrlConnectionNetworkFetcherProxied() {
		this(null,null, null, RealtimeSinceBootClock.get());
	}
	
	public HttpUrlConnectionNetworkFetcherProxied( int httpConnectionTimeout) {
		this(null, null, null, RealtimeSinceBootClock.get());
		mHttpConnectionTimeout = httpConnectionTimeout;
	}
	
	public HttpUrlConnectionNetworkFetcherProxied( Proxy proxy, String userAgent, int httpConnectionTimeout) {
		this(proxy, userAgent, null, RealtimeSinceBootClock.get());
		mHttpConnectionTimeout = httpConnectionTimeout;
	}
	
	public HttpUrlConnectionNetworkFetcherProxied(
		Proxy proxy, String userAgent, @Nullable Map<String, String> requestHeaders, int httpConnectionTimeout) {
		this(proxy, userAgent, requestHeaders, RealtimeSinceBootClock.get());
		mHttpConnectionTimeout = httpConnectionTimeout;
	}
	
	@VisibleForTesting
	HttpUrlConnectionNetworkFetcherProxied(
	...
	private HttpURLConnection downloadFrom( Uri uri, int maxRedirects) throws IOException {
		HttpURLConnection connection = openConnectionTo(uri, mProxy);
		if (mUserAgent != null) {
	...
	@VisibleForTesting
	protected HttpURLConnection openConnectionTo(Uri uri, Proxy proxy) throws IOException {
